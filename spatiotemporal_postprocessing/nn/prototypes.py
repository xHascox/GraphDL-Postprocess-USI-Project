...
class RNN_Model_1(nn.Module):
    def __init__(self, num_layers, input_size, output_dist, hidden_channels, n_stations, dropout_p=0.2, rnn_type='LSTM',**kwargs):
        super().__init__()
        ...
        self.rearrange_for_rnn = Rearrange('b t n c -> t (b n) c')
        self.rearrange_for_output = Rearrange('(b n) t c -> b t n c', n=n_stations)
        self.rearrange_for_output = Rearrange('t (b n) c -> b t n c', n=n_stations)
        ...
        for i in range(num_layers):
            input_size = hidden_channels[0] if i == 0 else hidden_channels[i - 1]
            rnn_layer = rnn_class(input_size=input_size, hidden_size=hidden_channels[i], num_layers=1, dropout=dropout_p if i < num_layers - 1 else 0)
    ...
    def forward(self, x, edge_index):
        x = self.encoder(x)
        x = x + self.station_embeddings()
        x = self.rearrange_for_rnn(x)
        
        for rnn_l in self.rnn_layers:
            x, _ = rnn_l(x)  # Passing through RNN layers
        
        x = self.rearrange_for_output(x)
